...
> **ix-init-cli 通过上面的 `<!-- IX_GIT_MODE_BEGIN/END -->` 标记区定位并改写本段**，仅替换标记区内容，不动文件其余部分。
...
| 目录 | 用途 |
|------|------|
| `.claude/` | Claude Code 配置：`rules/`、`settings.json`、`skills/`（待迁移） |
| `.wok-plans/` | wok-plans 工具配置（如使用） |

这些目录**不**属于「工作桶」，禁止在其中放业务交付物。

## 根目录白名单（重申）

仅允许：`CLAUDE.md`、`VERSION`、`.gitignore`、`.claude/`、`.wok-plans/`，以及五个工作桶（`_shared/`、`reports/`、`research/`、`artifacts/`、`ix-agents/`）。
