...
定时功能已独立为 [`ix-schedule-cli`](../../artifacts/ix-schedule-cli/)（跨平台：Windows schtasks / macOS launchd）。
...
需组合业务 Agent 时，**必须先跑 search 命令**发现已有能力：

```bash
python artifacts/ix-workspace-index-cli/main.py search "业务意图关键词"
```

有匹配的 `ix-*-agent` → Read 其 SPEC.yaml → 两阶段执行；无匹配 → 新建（见下方新建清单）。
