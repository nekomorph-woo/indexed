...
**仅下列「桶」必须有 `README.md`**（人类/Agent 速览，非权威；细节以 CLAUDE.md 为准）：
...
- `research/<topic>/`（专题根 README **可选**，见 `research.md`）
- `artifacts/<artifact-name>/`（工具根 `SPEC.md` + `SPEC.yaml` **必须**，见 `artifacts.md`）
- `ix-agents/<agent-name>/`（应用根 `SPEC.md` + `SPEC.yaml` **必须**，见 `ix-agents.md`）
