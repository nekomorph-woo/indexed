  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, overflowY: "auto", height: "100%" }}>
      <Card style={{ padding: 18 }}>
        <h3 style={{ margin: "0 0 4px", fontSize: 17 }}>主题皮肤</h3>
        <p style={{ margin: "0 0 16px", fontSize: 14, color: "var(--ix-text-muted)" }}>
              ...
              >
                ...
                <div style={{ padding: "8px 12px", background: "var(--ix-surface)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                    <span style={{ fontSize: 14, fontWeight: 600, color: active ? "var(--ix-primary)" : "var(--ix-text)" }}>
                      {dl.id}
                    </span>
                    {active && <Badge tone="primary">当前</Badge>}
                  </div>
                  <div style={{ fontSize: 12, color: "var(--ix-text-muted)" }}>{dl.name}</div>
      ...
      <Card style={{ padding: 18 }}>
        <h3 style={{ margin: "0 0 4px", fontSize: 17 }}>⚖️ 零侵入铁律</h3>
        <p style={{ margin: "0 0 12px", fontSize: 14, color: "var(--ix-text-muted)" }}>
          GUI 对工作区的字节级改动 = claude code 做同样的事。两种方式并存互通。
        </p>
        <ol style={{ margin: 0, paddingLeft: 20, fontSize: 14, color: "var(--ix-text)", lineHeight: 1.8 }}>
        ...
        <div style={{ marginTop: 12, padding: 12, background: "var(--ix-surface-alt)", borderRadius: "var(--ix-radius-sm)", fontSize: 13, color: "var(--ix-text-muted)" }}>
          详见 <code className="ix-mono">ix-gui/OVERVIEW.md</code> §零侵入铁律 + 三边界
        </div>
      </Card>

      <Card style={{ padding: 18 }}>
        <h3 style={{ margin: "0 0 4px", fontSize: 17 }}>当前阶段</h3>
        <p style={{ margin: "0 0 8px", fontSize: 14, color: "var(--ix-text-muted)" }}>
          阶段 1（纯 Web）：UI + 交互 + mock 契约，bb-browser 验证
        </p>
        <p style={{ margin: 0, fontSize: 14, color: "var(--ix-text-muted)" }}>
